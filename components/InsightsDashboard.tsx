...
function FrequencyWarning({ freq }: { freq: number }) {
  ...
  return (
    <div className="alert-banner" style={{ background: freq > 4 ? 'rgba(255,51,102,0.12)' : 'rgba(255,184,0,0.10)', border: `1px solid ${freq > 4 ? 'rgba(255,51,102,0.4)' : 'rgba(255,184,0,0.4)'}`, borderRadius: 10, padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
...
export function InsightsDashboard({ account, isLoading, datePreset, campaignMode = 'ecommerce', onSelectCampaign }: InsightsDashboardProps) {
  if (isLoading) {
    return (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      {campaignMode === 'whatsapp' && o && (o.messagesStarted > 0 || o.messagingFirstReply > 0) && (
        <div className="glass-panel">
          ...
          {o.costPerConversation > 0 && (
            <div style={{ marginTop: 20, padding: '12px 16px', background: 'rgba(0,240,255,0.07)', borderRadius: 10, fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
              💡 <strong style={{ color: 'var(--accent-primary)' }}>Custo por conversa:</strong> {formatCurrency(o.costPerConversation, account.currency)} ·
              <strong style={{ color: 'var(--accent-primary)' }}>Custo por 1º reply:</strong> {o.costPerFirstReply > 0 ? formatCurrency(o.costPerFirstReply, account.currency) : 'N/A'}
            </div>
          )}
        </div>
      )}
      ...
      <div className="glass-panel" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ margin: 0, fontSize: '1.1rem' }}>📢 Campanhas</h3>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{activeCampaigns.length} ativas de {campaigns.length} total</span>
          </div>
        </div>
              ...
              {campaigns.length === 0 ? (
                ...
                campaigns.map((campaign) => {
                  ...
                  return (
                    ...
                    >
                      <td>
                        <div style={{ fontWeight: 600, fontSize: '0.9rem', maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {campaign.name}
                        </div>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{campaign.id}</div>
                      </td>
                      ...
                      {campaignMode === 'ecommerce' && (
                        <>
                          <td style={{ color: ins?.roas ? getRoasColor(ins.roas) : 'var(--text-muted)', fontWeight: 700 }}>{ins?.roas ? `${ins.roas.toFixed(2)}x` : '-'}</td>
                      ...
                      {campaignMode === 'whatsapp' && (
                        <>
                          ...
                          <td style={{ color: ins?.costPerConversation && ins.costPerConversation < 20 ? 'var(--success)' : 'var(--warning)' }}>{ins?.costPerConversation ? formatCurrency(ins.costPerConversation, account.currency) : '-'}</td>
                        </>
                      )}
                      {campaignMode === 'leads' && (
                        <>
                          <td style={{ color: 'var(--accent-secondary)', fontWeight: 700 }}>{ins?.leads ? formatNumber(ins.leads) : '-'}</td>
                          <td style={{ color: ins?.costPerLead && ins.costPerLead < 50 ? 'var(--success)' : 'var(--warning)' }}>{ins?.costPerLead ? formatCurrency(ins.costPerLead, account.currency) : '-'}</td>
                        </>
                      )}
                      {campaignMode === 'traffic' && (
                        <>
                      ...
                      <td style={{ color: ins?.ctr ? (ins.ctr >= 1 ? 'var(--success)' : ins.ctr >= 0.5 ? 'var(--warning)' : 'var(--danger)') : 'var(--text-muted)' }}>{ins?.ctr ? `${ins.ctr.toFixed(2)}%` : '-'}</td>
                      <td style={{ color: ins?.frequency ? getFrequencyColor(ins.frequency) : 'var(--text-muted)', fontWeight: 600 }}>{ins?.frequency ? `${ins.frequency.toFixed(1)}x` : '-'}</td>
                      <td style={{ color: 'var(--text-secondary)' }}>{ins?.cpm ? formatCurrency(ins.cpm, account.currency) : '-'}</td>
                    </tr>
                  );
                })
              )}
